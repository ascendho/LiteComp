...
bool test_let_statement(const Node &s, const std::string &name, T value) {
    if (s.token_literal() != "declare") {
        std::cerr << "s->token_literal not 'declare'. got=" << s.token_literal() << std::endl;
        return false;
    }

    // Can now cast Node to a derived DeclareStatement, as we are confident that it is one
    auto let_stmt = dynamic_cast<const DeclareStatement *>(&s);
...
bool test_integer_literal(std::shared_ptr<Expression> il, int value) {
    ...
    if (integ->token.type != TokenType::INTEGER) {
        std::cerr << "ident->token.type not INTEGER. got=" << searchToken(integ->token.type) << std::endl;
        return false;
    }
...
bool test_identifier(std::shared_ptr<Expression> exp, std::string value) {
    ...
    if (ident->token.type != TokenType::IDENTIFIER) {
        std::cerr << "ident->token.type not IDENTIFIER. got=" << searchToken(ident->token.type) << std::endl;
        return false;
    }
...
bool test_boolean_expression(std::shared_ptr<Expression> exp, bool value) {
    ...
    if (!((bo->token.type == TokenType::TRUE) || (bo->token.type == TokenType::FALSE))) {
        std::cerr << "ident->token.type not TRUE or FALSE. got=" << searchToken(bo->token.type) << std::endl;
        return false;
    }
...
TEST_CASE("Test Function Literal Parsing") {
    ...
    // Cast Expression to a FuncLiteral, as this is what we are expecting
    auto function = std::dynamic_pointer_cast<FuncLiteral>(expression_stmt->expression);

    // Check that we have a FuncLiteral by checking if the dynamic pointer cast fails (returns nullptr)
    if (!function) {
        std::cerr << "function is not a FuncLiteral." << std::endl;
    }
    REQUIRE(function);
...
TEST_CASE("Test Function Parameter Parsing") {
    std::vector<std::tuple<std::string, std::vector<std::string>>> tests = {
            std::make_tuple("func() {};", std::vector<std::string>{}),
            std::make_tuple("func(x) {};", std::vector<std::string>{"x"}),
            std::make_tuple("func(x, y, z) {};", std::vector<std::string>{"x", "y", "z"}),
    };
...
TEST_CASE("Test Function Literal With Name") {
    std::string input = R"(declare myFunction = func() { };)";
    ...
    // Can now cast Node to a derived DeclareStatement, as we are confident that it is one
    auto let_stmt = std::dynamic_pointer_cast<DeclareStatement>(stmt);

    // Check that we have a Let Statement by checking if the dynamic pointer cast fails (returns nullptr)
    if (!let_stmt) {
        std::cerr << "program.statements.at(0) is not a DeclareStatement." << std::endl;
    }
    REQUIRE(let_stmt);

    // Cast statement value to a FuncLiteral, as this is what we are expecting
    auto function = std::dynamic_pointer_cast<FuncLiteral>(let_stmt->value);

    // Check that we have a HashLiteral by checking if the dynamic pointer cast fails (returns nullptr)
    if (!function) {
        std::cerr << "stmt->value is not a FuncLiteral." << std::endl;
    }
    REQUIRE(function);
